...
function QuoteRequestsPage({ user, profile, authReady }) {
    ...
    return (
      <main className="mx-auto min-h-screen w-full max-w-[1080px] px-6 pb-20 pt-28 sm:px-10">
        <section className="rounded-2xl border border-[var(--line)] bg-white p-8">
          <p className="text-sm text-[var(--muted)]">견적요청 내역을 불러오는 중입니다...</p>
        </section>
      </main>
    );
  }
      ...
      <section className="rounded-2xl border border-[var(--line)] bg-white p-6 shadow-[0_20px_44px_-34px_rgba(15,23,42,0.45)] sm:p-8">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h1 className="mt-1 font-brand text-3xl tracking-[0.05em] text-[var(--navy)]">견적 요청 확인</h1>
            <p className="mt-2 text-sm text-[var(--muted)]">검토 후 회신되는 견적 안내 문구를 확인할 수 있습니다.</p>
          </div>
          <Link
            to="/"
            ...
            quoteRequests.map((request) => (
              <article key={request.id} className="rounded-xl border border-[var(--line)] bg-[#fcfdff] p-5">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className="rounded-md border border-sky-300 bg-sky-50 px-2.5 py-1 text-xs font-semibold text-sky-700">
                    {request.status || '견적요청'}
                  </span>
                </div>
